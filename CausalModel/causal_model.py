...
class CausalModel:
    ...
    def learn_fci(self, df, tabu_edges, depth):
        """This function is used to learn model using FCI"""
        df = np.array(df)
        #添加了深度
        G, edges = fci(df, fisherz, 0.05, depth, verbose=False)
        nodes = G.get_nodes()   
        ...
        try:
            G, edges = fci(df, fisherz, 0.05, depth,verbose=False, background_knowledge=bk)
        except:
            G, edges = fci(df, fisherz, 0.05, depth, verbose=False)
        fci_edges = []
        for edge in edges:
    ...
    def compute_individual_treatment_effect(self, df, paths, 
        ...
        if len(objectives) >= 2:
            ...
            if len(selected_effect) > 0:
                ...
                print("ite[selected_index][0]:", ite[selected_index][0])
